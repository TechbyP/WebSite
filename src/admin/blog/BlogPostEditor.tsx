...
const BlogPostEditor = () => {
      ...
      try {
        ...
        querySnapshot.forEach((doc) => {
          ...
          articlesData.push({
            ...
            ...data,
            id: doc.id,
            content: { en: data.content?.en || defaultArticle.content?.en, de: data.content?.de || defaultArticle.content?.de }
          })
        })
  ...
  const uploadImage = async (file: File) => {
    ...
    } catch (error) { console.error('Image upload error:', error); throw error }
    finally { setIsLoading(false) }
  }
  ...
  const selectArticle = (article: Article) => { setSelectedArticle(article); setIsEditing(false); setIsCreating(false) }
  const startEditing = (article: Article) => {
    setFormData({ ...article, content: { en: article.content.en || { title: '', content: [''], excerpt: '' }, de: article.content.de || { title: '', content: [''], excerpt: '' } } })
    setIsEditing(true); setIsCreating(false); setSelectedArticle(null)
  }
  const startCreating = () => { setFormData(defaultArticle); setIsCreating(true); setIsEditing(false); setSelectedArticle(null) }

  const saveArticle = async (article: Partial<Article>) => {
    setIsLoading(true); setError(null)
    try {
      const articleToSave: Article = { ...article, id: article.id || Date.now().toString(), date: article.date || new Date().toISOString(), content: { en: article.content?.en || { title: '', content: [''], excerpt: '' }, de: article.content?.de || { title: '', content: [''], excerpt: '' } } } as Article
      ...
      setIsCreating(false); setIsEditing(false); setSelectedArticle(articleToSave)
    } catch (error) { console.error('Error saving article:', error); toast.error(error instanceof Error ? error.message : 'Failed to save article') }
    finally { setIsLoading(false) }
  }
  ...
  const deleteArticle = async (id: string) => {
    toast.info(
        ...
        <div className="flex justify-center gap-4 mt-4">
          <button
            onClick={async () => {
              ...
              } catch (error) { console.error(error); toast.error(error instanceof Error ? error.message : 'Failed to delete article') }
              finally { setIsLoading(false) }
            }}
      ...
      { position: "top-center", autoClose: false, closeButton: false, draggable: false, closeOnClick: false, theme: theme === 'dark' ? 'dark' : 'light' }
    )
  }
