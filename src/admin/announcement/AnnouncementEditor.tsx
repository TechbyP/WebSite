...
const AnnouncementEditor = () => {
 ...
 return (
   <div className="max-w-7xl mx-auto p-4 md:p-8 bg-white dark:bg-gray-900 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
  <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
    {/* Announcements List */}
    <div className="lg:col-span-1 space-y-4">
      <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">{t('editor.announcements')}</h2>
      <button
      ...
      <div className="space-y-3 max-h-[calc(100vh-250px)] overflow-y-auto pr-2">
        {announcements.map(announcement => {
          const content = announcement.content?.en || { title: '', subtitle: '', tag: '' };
            ...
            <motion.div
              ...
              className={`p-4 rounded-lg cursor-pointer border-2 transition-all ${
                currentAnnouncement.id === announcement.id
                  ? 'border-brandblue bg-blue-50 dark:bg-blue-900'
                  : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-500'
              }`}
            >
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-bold text-gray-900 dark:text-gray-100 truncate">{content.title}</h3>
                  <p className="text-sm text-gray-600 dark:text-gray-300">{content.subtitle}</p>
                </div>
                <span
              ...
              <div className="flex justify-between items-center mt-2">
                ...
                <button
                  ...
                  className="text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-600 text-sm font-medium"
                >
                  {t('editor.delete')}
                </button>
              </div>
      ...
      <div className="flex mb-6">
        <button
          ...
          className={`px-4 py-2 rounded-l-lg font-medium ${
            currentLanguage === 'en' ? 'bg-brandblue text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200'
          }`}
        >
          English
        </button>
        <button
          ...
          className={`px-4 py-2 rounded-r-lg font-medium ${
            currentLanguage === 'de' ? 'bg-brandblue text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200'
          }`}
        >
          Deutsch
        </button>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-6">
            {['tag', 'title', 'subtitle', 'dateInfo', 'location'].map(field => (
              <div key={field}>
                <label htmlFor={field} className="block text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">
                  {t(`editor.${field}`)}
                </label>
                <input
                  type="text"
                  id={field}
                  name={field}
                  value={currentAnnouncement.content[currentLanguage][field as keyof AnnouncementContent]}
            ...
            <div>
              <label htmlFor="priority" className="block text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">{t('editor.priority')}</label>
              <select
                id="priority"
                name="priority"
          ...
          {/* CTA Fields */}
          <div className="space-y-6">
            {['ctaPrimary', 'ctaPrimaryLink', 'ctaSecondary', 'ctaSecondaryLink'].map(field => (
              <div key={field}>
                <label htmlFor={field} className="block text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">{t(`editor.${field}`)}</label>
                <input
          ...
          <div className="md:col-span-2">
            <label htmlFor="description" className="block text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">{t('editor.description')}</label>
            <textarea
              id="description"
              name="description"
          ...
          {/* Image Upload */}
          <div className="md:col-span-2">
            <label className="block text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">{t('editor.image')}</label>
            {currentAnnouncement.imageUrl && (
              <div className="mb-4 p-3 bg-green-50 dark:bg-green-900 border border-green-200 dark:border-green-700 rounded-lg">
                ...
                <button
                  ...
                  className="mt-2 text-sm text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-600 font-medium flex items-center"
                >
            ...
            {(imagePreview || currentAnnouncement.imageUrl) && (
              <div className="mt-4">
                <div className="text-md font-semibold text-gray-900 dark:text-gray-100 mb-2">
                ...
                <img
                  ...
                  alt="Preview"
                  className="w-full max-h-64 object-contain rounded-xl border-2 border-gray-200 dark:border-gray-700"
                />
              </div>
            )}
          </div>
          ...
          {/* Footer Buttons */}
          <div className="flex justify-end space-x-4 pt-6 md:col-span-2">
            <button
              type="button"
              onClick={handleNewAnnouncement}
              className="px-6 py-2.5 border-2 border-gray-300 dark:border-gray-600 rounded-lg text-lg font-semibold text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
            >
              {t('editor.new')}
            </button>
            <button
