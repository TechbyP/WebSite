...
const ProductToastContent = ({ product, t }: { product: Product; t: any }) => {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 20 }}
      className="relative bg-white rounded-2xl shadow-xl max-w-3xl w-full flex overflow-hidden font-sans border border-gray-200"
    >
      {/* Image Section */}
...
srcSet={getProductMediaFallbacks(product).heroImage}
      ...
      <div className="w-full md:w-1/2 p-6 flex flex-col justify-center relative">
        ...
        <div className="mb-4">
          ...
          <p className="text-sm text-gray-500">{product.nickname}</p>
        </div>

        {/* Description */}
        <div className="text-gray-700 text-sm space-y-3 leading-snug">
          <p>{translate('configurator.productToast.desc1')}</p>
          <p>
            {translate('configurator.productToast.desc2.part1')}{' '}
            <strong>{product.name || translate('configurator.productToast.machine')}</strong>{' '}
        ...
        >
          {translate('configurator.productToast.configure', { 
            productName: product.name || translate('configurator.productToast.machine')
          })} →
        </button>
      </div>
    </motion.div>
